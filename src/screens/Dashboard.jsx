...
export default function Dashboard({ store, setActivePage }) {
  ...
  return <Page>
    <header className="app-header beta-header"><div><span className="eyebrow">Freedom Travel OS · Public Edition</span><h1>{trip.traveller || "Traveller"}</h1></div><span className="status-chip">{days > 0 ? `${days} days` : trip.status}</span></header>
