...
export default function Packing({ store }) {
  ...
  return <Page>
    ...
    <Card className="compact-form-card"><form className="packing-add-form" onSubmit={submit}><input placeholder="Item name" value={draft.label} onChange={e=>setDraft({...draft,label:e.target.value})}/><select value={draft.category} onChange={e=>setDraft({...draft,category:e.target.value})}>{CATEGORIES.map(c=><option key={c}>{c}</option>)}</select><div className="packing-mini-grid"><input type="number" min="1" placeholder="Qty" value={draft.quantity} onChange={e=>setDraft({...draft,quantity:e.target.value})}/><input type="number" min="0" step="0.1" placeholder="Weight kg" value={draft.weightKg} onChange={e=>setDraft({...draft,weightKg:e.target.value})}/></div><div className="packing-flags"><label><input type="checkbox" checked={draft.essential} onChange={e=>setDraft({...draft,essential:e.target.checked})}/> Essential</label><label><input type="checkbox" checked={draft.toBuy} onChange={e=>setDraft({...draft,toBuy:e.target.checked})}/> Need to buy</label></div><button><PackagePlus size={16}/> Add Item</button></form></Card>
    ...
    <ModalSheet title="Edit Packing Item" open={Boolean(editing)} onClose={()=>setEditing(null)}>{editing&&<form className="packing-add-form" onSubmit={saveEdit}><input placeholder="Item name" value={editing.label} onChange={e=>setEditing({...editing,label:e.target.value})}/><select value={editing.category} onChange={e=>setEditing({...editing,category:e.target.value})}>{CATEGORIES.map(c=><option key={c}>{c}</option>)}</select><div className="packing-mini-grid"><input type="number" min="1" value={editing.quantity||1} onChange={e=>setEditing({...editing,quantity:e.target.value})}/><input type="number" min="0" step="0.1" value={editing.weightKg||""} placeholder="Weight kg" onChange={e=>setEditing({...editing,weightKg:e.target.value})}/></div><input placeholder="Optional note" value={editing.meta||""} onChange={e=>setEditing({...editing,meta:e.target.value})}/><div className="packing-flags"><label><input type="checkbox" checked={Boolean(editing.essential)} onChange={e=>setEditing({...editing,essential:e.target.checked})}/> Essential</label><label><input type="checkbox" checked={Boolean(editing.toBuy)} onChange={e=>setEditing({...editing,toBuy:e.target.checked})}/> Need to buy</label></div><button><Pencil size={16}/> Save Changes</button></form>}</ModalSheet>
  </Page>;
}
