...
export default function Memories({ store }) {
  const { trip, memories, addMemory, toggleMemoryFavourite, deleteMemory } = store;
  const [title, setTitle] = useState(""); const [note, setNote] = useState(""); const [date, setDate] = useState(trip.startDate || new Date().toISOString().slice(0,10)); const [photo, setPhoto] = useState(""); const [busy, setBusy] = useState(false);
