...
export default function Documents({store}){
  ...
  <ModalSheet title={editing?"Edit document":"Add document"} open={open} onClose={()=>setOpen(false)} footer={<button className="primary-button full-width-action" onClick={save}>Save document</button>}><div className="sheet-form"><label>Type<select value={form.type} onChange={e=>setForm({...form,type:e.target.value})}>{TYPES.map(x=><option key={x}>{x}</option>)}</select></label><label>Document name<input value={form.title} onChange={e=>setForm({...form,title:e.target.value})} placeholder="e.g. Travel insurance"/></label><label>Reference / location<input value={form.reference} onChange={e=>setForm({...form,reference:e.target.value})} placeholder="Policy number or where it is saved"/></label><label>Expiry date<input type="date" value={form.expiry||""} onChange={e=>setForm({...form,expiry:e.target.value})}/></label><label>Notes<textarea value={form.notes||""} onChange={e=>setForm({...form,notes:e.target.value})} placeholder="Optional notes"/></label></div></ModalSheet></Page>;
}
