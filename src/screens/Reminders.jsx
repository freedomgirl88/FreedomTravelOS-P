...
export default function Reminders({store}){
  ...
  return <Page><header className="app-header"><div><span className="eyebrow">Trip Alerts</span><h1>Reminders</h1></div><button className="status-chip" onClick={()=>launch()}><Plus size={15}/> Add</button></header>
    ...
    <ModalSheet title={editing?"Edit Reminder":"Create Reminder"} open={open} onClose={()=>setOpen(false)}><form className="inline-form" onSubmit={save}>
      <input required placeholder="Reminder title" value={form.title} onChange={e=>setForm(v=>({...v,title:e.target.value}))}/>
      <select value={form.type} onChange={e=>setForm(v=>({...v,type:e.target.value}))}><option>General</option><option>Flight</option><option>Hotel</option><option>Booking</option><option>Packing</option><option>Transport</option><option>Activity</option></select>
      <label className="field"><span>Date</span><input type="date" value={form.date} onChange={e=>setForm(v=>({...v,date:e.target.value}))}/></label>
      <label className="field"><span>Time</span><input type="time" value={form.time} onChange={e=>setForm(v=>({...v,time:e.target.value}))}/></label>
      <textarea rows="3" placeholder="Optional note" value={form.note} onChange={e=>setForm(v=>({...v,note:e.target.value}))}/>
      <button><BellRing size={16}/> {editing?"Save Changes":"Create Reminder"}</button>
    </form></ModalSheet>
  </Page>
}
