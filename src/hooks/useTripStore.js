...
import { defaultTrip, defaultPacking, defaultExpenses, defaultExploreDays, defaultBookingHistory, defaultReminders } from "../data/defaults";
...
function createTripRecord(tripData = {}) {
  ...
  return {
    ...
    documents: [],
    emergencyContacts: [],
    packingSettings: { luggageLimitKg: 10, tripType: "Vacation" }
  };
}
...
export function useTripStore(){
  ...
  const { trip, packing, expenses, days, selectedDay, visited, favourites, bookings, memories, reminders = [], documents = [], emergencyContacts = [], packingSettings = { luggageLimitKg: 10, tripType: "Vacation" } } = record;
  ...
  const spentSGD=expenses.reduce((sum,e)=>sum+krwToSgd(e.amountKRW,Number(e.rateUsed)>0?e.rateUsed:trip.exchangeRate),0);
  ...
  const clearFlight=(leg)=>updateActive(item=>({...item,trip:{...item.trip,[leg==="return"?"returnFlight":"flight"]:{...clone(defaultTrip.flight)}}}));
  const clearHotel=()=>updateActive(item=>({...item,trip:{...item.trip,hotel:{...clone(defaultTrip.hotel)}}}));
  ...
  const updatePackingSettings=updates=>updateActive(item=>({...item,packingSettings:{...(item.packingSettings||{luggageLimitKg:10,tripType:"Vacation"}),...updates}}));
  const replacePackingList=list=>updateActive(item=>({...item,packing:list}));
  const addExpense=e=>updateActive(item=>({...item,expenses:[...item.expenses,{id:uid("expense"),...e}]}));
  const updateExpense=(id,updates)=>updateActive(item=>({...item,expenses:item.expenses.map(e=>e.id===id?{...e,...updates}:e)}));
  ...
  const updateBooking=(id,updates)=>updateActive(item=>({...item,bookings:item.bookings.map(b=>b.id===id?{...b,...updates}:b)}));
  ...
  const addReminder=r=>updateActive(item=>({...item,reminders:[...(item.reminders||[]),{id:uid("reminder"),done:false,createdAt:new Date().toISOString(),...r}]}));
  const updateReminder=(id,updates)=>updateActive(item=>({...item,reminders:(item.reminders||[]).map(r=>r.id===id?{...r,...updates}:r)}));
  const toggleReminder=id=>updateActive(item=>({...item,reminders:(item.reminders||[]).map(r=>r.id===id?{...r,done:!r.done}:r)}));
  const deleteReminder=id=>updateActive(item=>({...item,reminders:(item.reminders||[]).filter(r=>r.id!==id)}));


  const addDocument=d=>updateActive(item=>({...item,documents:[...(item.documents||[]),{id:uid("document"),createdAt:new Date().toISOString(),...d}]}));
  const updateDocument=(id,updates)=>updateActive(item=>({...item,documents:(item.documents||[]).map(d=>d.id===id?{...d,...updates}:d)}));
  const deleteDocument=id=>updateActive(item=>({...item,documents:(item.documents||[]).filter(d=>d.id!==id)}));
  const addEmergencyContact=c=>updateActive(item=>({...item,emergencyContacts:[...(item.emergencyContacts||[]),{id:uid("contact"),...c}]}));
  const updateEmergencyContact=(id,updates)=>updateActive(item=>({...item,emergencyContacts:(item.emergencyContacts||[]).map(c=>c.id===id?{...c,...updates}:c)}));
  const deleteEmergencyContact=id=>updateActive(item=>({...item,emergencyContacts:(item.emergencyContacts||[]).filter(c=>c.id!==id)}));
  ...
  return {workspace,trips:workspace.trips.map(({id,trip,updatedAt})=>({id,trip,updatedAt})),activeTripId:workspace.activeTripId,hasTrips:workspace.trips.length>0,trip,packing,expenses,days,selectedDay,setSelectedDay,visited,favourites,bookings,memories,packed,packingProgress,spentSGD,remainingSGD,activeDay,activeVisited,exploreProgress,ready,createTrip,switchTrip,deleteTrip,updateTripField,updateFlightField,updateHotelField,updateReturnFlightField,clearFlight,clearHotel,togglePacking,addPackingItem,updatePackingItem,deletePackingItem,updatePackingSettings,replacePackingList,packingSettings,addExpense,updateExpense,deleteExpense,toggleVisited,toggleFavourite,addBooking,updateBooking,deleteBooking,addMemory,updateMemory,toggleMemoryFavourite,deleteMemory,reminders,addReminder,updateReminder,toggleReminder,deleteReminder,documents,addDocument,updateDocument,deleteDocument,emergencyContacts,addEmergencyContact,updateEmergencyContact,deleteEmergencyContact,addExploreDay,updateExploreDay,deleteExploreDay,addExplorePlace,updateExplorePlace,deleteExplorePlace,moveExplorePlace,resetActiveTrip,resetAll};
}
