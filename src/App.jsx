...
export default function App(){
  ...
  const [showSplash,setShowSplash]=useState(()=>sessionStorage.getItem("ftos-public-splash-seen")!=="1");
  const store=useTripStore();
  const finishSplash=useCallback(()=>{sessionStorage.setItem("ftos-public-splash-seen","1");setShowSplash(false);},[]);

  useEffect(()=>{const apply=()=>{const resolved=theme==="system"?(window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"):theme;document.body.dataset.theme=resolved;document.body.dataset.themePreference=theme;};apply();localStorage.setItem("ftos-theme-v2",theme);const media=window.matchMedia("(prefers-color-scheme: dark)");media.addEventListener?.("change",apply);return()=>media.removeEventListener?.("change",apply);},[theme]);
