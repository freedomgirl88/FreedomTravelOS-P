...
const VALID_PAGES = new Set(["dashboard","flight","hotel","packing","budget","explore","booking","more","settings","memories","reminders","release-notes","documents","emergency","timeline"]);
...
export default function App(){
  ...
  const screen=activePage==="dashboard"?<Dashboard {...sharedProps}/>:activePage==="flight"?<Flight store={store}/>:activePage==="hotel"?<Hotel store={store}/>:activePage==="packing"?<Packing store={store}/>:activePage==="budget"?<Budget store={store}/>:activePage==="explore"?<Explore store={store}/>:activePage==="booking"?<Booking store={store}/>:activePage==="more"?<More setActivePage={setActivePage} theme={theme} setTheme={setTheme}/>:activePage==="settings"?<Settings store={store} theme={theme} setTheme={setTheme}/>:activePage==="memories"?<Memories store={store}/>:activePage==="reminders"?<Reminders store={store}/>:activePage==="release-notes"?<ReleaseNotes updater={updater}/>:activePage==="documents"?<Documents store={store}/>:activePage==="emergency"?<Emergency store={store}/>:activePage==="timeline"?<Timeline store={store}/>:<Dashboard {...sharedProps}/>;

  if(!store.trip.isConfigured)return <div className={`app-shell${showSplash?" splash-active":""}`}><div className={`phone-frame${showSplash?" splash-active":""}`}>{showSplash&&<SplashScreen onDone={finishSplash}/>}<CreateTrip store={store}/><WhatsNewModal enabled={!showSplash}/><UpdateBanner open={updater.updateAvailable&&!showSplash} onUpdate={updater.installUpdate} onLater={updater.dismissUpdate}/>{toast&&<div className={`toast toast-${toast.type}`}>{toast.message}</div>}</div></div>;
  return <div className={`app-shell${showSplash?" splash-active":""}`}><a className="skip-link" href="#main-content">Skip to content</a><div className={`phone-frame${showSplash?" splash-active":""}`}>{showSplash&&<SplashScreen onDone={finishSplash}/>}<div id="main-content" className="screen-transition" key={activePage}>{screen}</div><BottomNav activePage={activePage} setActivePage={setActivePage}/><WhatsNewModal enabled={!showSplash}/><UpdateBanner open={updater.updateAvailable&&!showSplash} onUpdate={updater.installUpdate} onLater={updater.dismissUpdate}/>{toast&&<div className={`toast toast-${toast.type}`} key={toast.id}>{toast.message}</div>}</div></div>;
}
